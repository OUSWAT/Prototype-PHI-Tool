...
if($archive == 1 || $analysis){
	...
	elseif(substr($d,0,8) == '20160509' || substr($d,0,8) == '20160510'){
                $caseDir = '20160509';
                $site = 'KTLX';
        }
	elseif(substr($d,0,8) == '20160524' || substr($d,0,10) == '2016052501'){
                $caseDir = '20160524';
                $site = 'KDDC';
        }
	elseif(substr($d,0,8) == '20160525' || substr($d,0,8) == '20160526'){
                $caseDir = '20160525';
                $site = 'KTWX';
        }
...
foreach($files as $f){
	...
	$autoEnd = 999999999999999;
		...
		if($geojson->properties->data->automated == 0){
			...
			$mFound = false;
			foreach($geojson->properties->data->modified as $mKey => $mValue){
                               	if($mValue == 0){
                                        $mFound = true;
                                       	break;
                               	}
			...
			if($mFound){
				if(in_array($id,$blockKeys) && $geojson->properties->data->valid_start < $blockIDs[sprintf('%07d',$id)][0]){
	                                $geojson->properties->data->allow = 0;
				}
				$modifiedObjects[sprintf('%07d',$id)] = $geojson; // object is level 2
			...
			elseif($geojson->properties->data->valid_end < $inTime){
				break; // latest human object has expired
			}
			else{
				if(in_array($id,$blockKeys) && $geojson->properties->data->valid_start < $blockIDs[sprintf('%07d',$id)][0]){
                                       	$geojson->properties->data->allow = 0;
	...
	foreach($geojsons as $line){
		...
		if(in_array($id,$keys)){
			...
			if($modifiedObjects[sprintf('%07d',$id)]->properties->data->modified->object == 1){
				...
				$geojson->properties->data->valid_start = $modifiedObjects[sprintf('%07d',$id)]->properties->data->valid_start;
				$geojson->properties->data->valid_end = $geojson->properties->data->valid_start + (60 * $geojson->properties->data->duration);
                        }
                        else{
			...
			$geojson->properties->data->issue = $modifiedObjects[sprintf('%07d',$id)]->properties->data->issue;
...
foreach($keys as $modID){
	if(!in_array($modID,$usedKeys)){
		...
		if($modifiedObjects[$modID]->properties->data->modified->object == 1){
			...
			$geojson->properties->data->modified->object = 1;  // converts from L3 to L2
	               	$geojson->properties->data->automated = 0; // converts from L3 to L2
			if($modifiedObjects[$modID]->properties->data->modified->objShape == 1){
                                $geojson->properties->data->buffer = $modifiedObjects[$modID]->properties->data->buffer;
                                $geojson->properties->data->bufDir = $modifiedObjects[$modID]->properties->data->bufDir;
                                $geojson->properties->data->automated = 2;
                        }
                        if($modifiedObjects[$modID]->properties->data->modified->objPosition == 1){
                                $geojson->properties->data->objOffsetX = $modifiedObjects[$modID]->properties->data->objOffsetX;
                                $geojson->properties->data->objOffsetY = $modifiedObjects[$modID]->properties->data->objOffsetY;
                                $geojson->properties->data->automated = 2;
                        }
		...
		$geojson->properties->data->valid_end = $geojson->properties->data->valid_start + (60 * $geojson->properties->data->duration);
		if($modifiedObjects[$modID]->properties->data->valid_start > $geojson->properties->data->valid_start){
			$geojson->properties->data->valid_start = $modifiedObjects[$modID]->properties->data->valid_start;
		}
		$geojson->properties->data->issue = $modifiedObjects[$modID]->properties->data->issue;
                $geojson->properties->data->allow = 1;
		$geojson->properties->data->init = -999;
