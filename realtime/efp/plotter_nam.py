...
#data_path  = 'http://nomads.ncep.noaa.gov/pub/data/nccf/com/nam/prod/nam.' + init[:-2]
#data_path = 'http://nomads.ncep.noaa.gov/cgi-bin/filter_nam.pl?lev_1000-0_m_above_ground=on&lev_10_m_above_ground=on&lev_255-0_mb_above_ground=on&lev_2_m_above_ground=on&lev_3000-0_m_above_ground=on&lev_5000-2000_m_above_ground=on&lev_entire_atmosphere_%5C%28considered_as_a_single_layer%5C%29=on&lev_surface=on&var_CAPE=on&var_CIN=on&var_DPT=on&var_HLCY=on&var_REFC=on&var_TMP=on&var_UGRD=on&var_UPHL=on&var_VGRD=on&var_APCP=on&leftlon=0&rightlon=360&toplat=90&bottomlat=-90'
data_path = 'http://nomads.ncep.noaa.gov/cgi-bin/filter_nam.pl?&all_lev=on&var_CAPE=on&var_CIN=on&var_DPT=on&var_HLCY=on&var_REFC=on&var_TMP=on&var_UGRD=on&var_VGRD=on&var_APCP=on&var_REFD=on&leftlon=0&rightlon=360&toplat=90&bottomlat=-90'
...
member = 'nam'
fPath = path + '/' + member
badHours = [37,38,40,41,43,44,46,47]

# main loop
for fHour in range(fStart,37):
	if fHour in badHours:
		continue
	if fHour >= (fStart + 2):
                cmd = 'python ' + path + '/plotter_nam.py ' + str(hour) + ' ' + str(fHour) + ' ' + init[:-2] + ' >& ' + path + '/out_' + member + '.txt &'
                print cmd
                os.system(cmd)
                sys.exit()
	...
	fname = 'nam.t' + init[-2:] + 'z.awphys' + str(fHour).zfill(2) + '.grb2.tm00'
	f_remote = data_path + '&dir=%2Fnam.' + init[:-2] + '&file=' + fname
	#f_remote = os.path.join(data_path, fname)
	f_local = os.path.join(fPath, fname)
	fields = ['precip03','precip06','precip12','t2m','td2m','refl','refl1km','hlcy03','mucape','cape','cin','uwind']
	not_here = True
	...
	for field in fields:
		...
		# get prior data for precip accum
			...
			fname = 'nam.t' + init[-2:] + 'z.awphys' + str(fHour - accumHours).zfill(2) + '.grb2.tm00'
		        f_local = os.path.join(fPath, fname)
                        try:
		...
		remote_dir = remote_base_dir + '/' + init
                ...
                ssh.connect('bigbang.protect.nssl',username='chris.karstens', password='1LickityOscar!')
                sftp = ssh.open_sftp()
                try:
